...
int main (void){
    ...
    // draw numbers until the last board gets a bingo
    for(int num : nums){

        boardsSkipped = 0;
        winningNumber = -1;
        unmarkedSum = 0;
        for(bool element : skipBoard) if (element) boardsSkipped++;

        for (int i = 0; i < numOfBoards; i++){
            if (skipBoard[i] == true){
                // cout << "Skipping board: " << i << " for num: " << num << endl;
                if(boardsSkipped < numOfBoards)
                    continue;
            }else{
                // cout << "Testing board: " << i << " for num: " << num << endl;
                for (int j = 0; j < boardSize; j++){
                    for (int k = 0; k < boardSize; k++){
                        if(board[i][j][k] == num){
                            filledBoard[i][j][k] = true;
                            // cout << "Testing board: " << i << " for bingo for num: " << num << endl;
            ...
            if(winningNumber != -1 && boardsSkipped == numOfBoards - 1) break;
        }
        if(winningNumber != -1 && boardsSkipped == numOfBoards - 1) break;
    }
